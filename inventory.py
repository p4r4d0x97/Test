...
DEFAULT_CONFIG = {
    ...
    "netmap": {
        ...
        "poe_vlans":  [],       # VLANs whose devices support PoE port cycling
...
class XMLInventoryManager:
    ...
    def _build(self, dev: RawDevice) -> ET.Element:
        ...
        for tag, val in [
            ...
            ("first-seen", now_str), ("history", ""),
            # Freshness markers:
            #   last-scanned = when the scanner last checked this device (online or not)
            #   last-online  = when the scanner last confirmed it as online (empty if never)
            ("last-scanned", now_str),
            ("last-online", now_str if dev.online else ""),
            ("xml_change_status", f"added:{now_str}"),
        ]:
            _sub(el, tag, val)
        return el
    ...
    def _update(self, el: ET.Element, dev: RawDevice) -> bool:
        now_str = _now()
        # Freshness markers — updated on EVERY scan attempt, regardless of
        # whether the device is online or any field actually changed.
        # This lets users distinguish "silence = network down" from
        # "silence = we haven't looked in weeks".
        scanned_el = el.find("last-scanned") or ET.SubElement(el, "last-scanned")
        scanned_el.text = now_str
        if dev.online:
            online_el = el.find("last-online") or ET.SubElement(el, "last-online")
            online_el.text = now_str
        # Offline devices: still record we tried, but don't process field updates
        # (we don't have fresh values to write anyway).
        if not dev.online:
            return False
        changed = False
