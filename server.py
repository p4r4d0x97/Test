...
def update_live_status(mac_results: dict, by_user: str = ""):
    ...
    with _live_status_lock:
        ...
        # cap size — drop entries older than 30 minutes (server-side freshness threshold)
        cutoff = now - 30*60
        ...
        if path == "/api/devices":
            ...
            try:
                ...
                self._json({"ok": True,
                            ...
                            "warn_bytes": 10 * 1024 * 1024 * 1024,   # 10 GB
                            "warn_reached": total > 10 * 1024 * 1024 * 1024})
            except Exception as e:
                self._json({"ok": False, "error": str(e)})
        ...
        if self.path == "/api/ping":
            try:
                ...
                if not macs:
                    ...
                    return
                result = do_ping(macs)
                # Publish results to shared file so other users benefit
                try: update_live_status(result, by_user)
                except Exception as _e: print(f"[live-status] {_e}")
                self._json(result)
            except Exception as e:
                self._error(str(e))
            ...
            try:
                ...
                try:
                    ...
                    if total + len(raw) > 10 * 1024 * 1024 * 1024:
                        gb = round((total + len(raw)) / 1024**3, 2)
                        self._json({"ok": False,
                                    "error": f"Photos folder would exceed 10 GB ({gb} GB after this upload). "
            ...
            # Overwrites site_config.json. Never contains credentials.
            try:
                data = json.loads(body)
